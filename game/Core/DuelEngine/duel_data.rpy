init python:
    ...
    def combo(card):
        ...
        if user.side == 'allies':
            ...
            max_ = 0
            card_to_get = None
            for card in user.user.draw_list():
                if card.power > max_:
                    card_to_get = card
                    max_ = card.power
    ...
    actions_lib = {'clinch': {'name': __('clinch'), 'rarity': 'base', 'power': 0, 'special_effect': clinch_special, 'on_remove': clinch_remove, 'description': __('Persistent effect. The maneuver do not scores for both combatants while clinch is in the game.'), },
                    'hit_n_run': {'name': __('hit-n-run'), 'rarity': 'base', 'power': 0, 'special_effect': hit_n_run_special, 'on_remove': hit_n_run_remove, 'description': __('Persistent effect. The onslaught do not scores for both combatants while hit-n-run is in the game.'), },
                    'rage': {'name': __('rage'), 'rarity': 'base', 'power': 0, 'special_effect': rage_special, 'on_remove': rage_remove, 'description': __('Persistent effect. The fortitude do not scores for both combatants while rage is in the game.'), },
                    'outsmart': {'name': __('outsmart'), 'rarity': 'base', 'power': 0, 'special_effect': outsmart_special, 'description': __('Discards all persistent effects.'), },
                    'fallback': {'name': __('fallback'), 'rarity': 'base', 'power': 0, 'special_effect': fallback_special, 'description': __('Deduct your current escalation from your biggest basic point-pool. Get one new card.'), },
                    ...
                    'savage_force': {'name': __('savage force'), 'rarity': 'base', 'power': 3, 'use_weapon': True, 'slot': 'versatile', 'description': __('{color=#0000ff}Weapon action{/color} \nVersatile - chose common victory points slot to add this cards power'), 'special_effect': None, 'power_mods': None, 'special_mechanics': [combo, sequence, amplifiction, advantage]}                    
